...
log = logging.getLogger("cozmo.agent.reflector")
...
class Lesson:
    ...
    def matches(self, tool: str, error_text: str) -> float:
        """Score how well this lesson matches a new error. 0-1."""
        score = 0.0
        if self.tool == tool:
            score += 0.4
...
class LessonStore:
    ...
    def format_for_prompt(self, tool: str, error_text: str) -> str:
        """Return relevant lessons formatted for prompt injection."""
...
class Reflector:
    ...
    def before_step(self, tool: str, args: dict) -> ReflectionResult:
        """Called before executing a tool. Checks for known failure patterns."""
        return ReflectionResult(success=True)

    def after_step(self, tool: str, args: dict, result: str) -> ReflectionResult:
        """Called after executing a tool. Classifies errors, recommends recovery."""
    ...
    def _suggest(self, tool: str, error_type: ErrorType, args: dict, result: str) -> tuple[RetryStrategy, str]:
        ...
        if error_type == ErrorType.NOT_FOUND:
            return RetryStrategy.CHANGE_TOOL, f"Path or resource not found. Verify the path exists."
        ...
        if error_type == ErrorType.VALIDATION:
            return RetryStrategy.MODIFY_PARAMS, f"Invalid arguments. Check the tool's parameter requirements."
        return RetryStrategy.CHANGE_TOOL, f"Tool failed. Try a different approach or tool."
