...
DATA_PATH = "data/"
PDF_FILENAME = "Robert's Resume - IT.pdf" 

def load_documents():
    """Loads docuemtns from the specified data path."""
    pdf_path = os.path.join(DATA_PATH, PDF_FILENAME)
    loader = PyPDFLoader(pdf_path)
    # loader = UnstructuredPDFLoader(pdf_path)  # For more complex PDFs
    documents = loader.load()
    print(f"Loaded {len(documents)} pages from {pdf_path}")
    return documents

def split_documents(documents):
    """SPlits documents into smaller chuncks."""
...
def get_embedding_function(model_name = "nomic-embed-text:latest"):
    """Initializes the Ollama embedding function."""
    # Ensure Ollama server is running (ollama serve)
    embeddings = OllamaEmbeddings(model=model_name)
    print(f"Intialized embedding function with model {model_name}")
    return embeddings
...
def index_documents(chunks, embedding_function, persist_directory=CHROMA_PATH):
    ...
    # This will overwrite exisitng data if the directory exists but isn't a valid Chroma DB.
...
def create_rag_chain(vector_store, llm_model_name="qwen3:8b", context_window=8192):
    ...
    print(f"Initialized ChatOllama with mdoel: {llm_model_name}, context window: {context_window}")
    ...
    # Define the RAG chain using LECLE
    rag_chain = (
